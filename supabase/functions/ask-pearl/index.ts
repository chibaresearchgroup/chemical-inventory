...
async function callModel(prompt: string): Promise<string> {
      ...
      `https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(model)}:generateContent`,
      {
        ...
        body: JSON.stringify({
          systemInstruction: {
            parts: [
              {
                text: 'You are Ask ChibaLab, a careful read-only lab inventory assistant. Answer only from the supplied JSON context. If the context is insufficient, say what is missing. Do not invent records, prices, safety rules, or file contents. Be concise and include specific record names/codes when useful.',
              },
            ],
          },
        ...
        messages: [
          {
            role: 'system',
            content:
              'You are Ask ChibaLab, a careful read-only lab inventory assistant. Answer only from the supplied JSON context. If the context is insufficient, say what is missing. Do not invent records, prices, safety rules, or file contents. Be concise and include specific record names/codes when useful.',
          },
  ...
  if (selected === 'ollama' || selected === 'local') {
    ...
    const upstream = await fetch(`${baseUrl.replace(/\/$/, '')}/api/generate`, {
      ...
      body: JSON.stringify({
        model,
        prompt: `You are Ask ChibaLab, a careful read-only lab inventory assistant. Answer only from the supplied JSON context. If the context is insufficient, say what is missing. Do not invent records, prices, safety rules, or file contents. Be concise and include specific record names/codes when useful.\n\n${prompt}`,
        stream: false,
...
Deno.serve(async (req) => {
  ...
  if (action === 'draft_methods') {
    ...
    const prompt = `Draft a concise first-pass methods section for the project below. Use only the supplied ChibaLab JSON. Keep it manuscript-ready but mark missing details as TODO. Include experimental materials and computational methods when present.\n\nScoped JSON context:\n${JSON.stringify(scoped)}`
    try {
