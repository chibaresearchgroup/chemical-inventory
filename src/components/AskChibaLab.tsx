...
import { api, type AskChibaLabReply } from '../lib/api'
import { Logo } from './Logo'
import { Spinner } from './ui'

type WidgetState = 'pill' | 'icon' | 'panel'
type Message = { role: 'user' | 'assistant'; text: string; sources?: AskChibaLabReply['sources'] }
...
function ChibaLabMark({ className = 'h-6 w-6' }: { className?: string }) {
  return (
    <span
...
export function AskChibaLab() {
  ...
  async function askLocal(trimmed: string): Promise<AskChibaLabReply> {
    ...
    const res = await fetch(`${localUrl.replace(/\/$/, '')}/api/generate`, {
      ...
      body: JSON.stringify({
        model: localModel,
        stream: false,
        prompt: `You are Ask ChibaLab, a careful read-only lab inventory assistant. The user is currently in the ${workspace} workspace. Answer only from the supplied JSON context. If the context is insufficient, say what is missing. Do not invent records, prices, safety rules, or file contents. Be concise and include specific record names/codes when useful.\n\nQuestion: ${trimmed}\n\nScoped JSON context:\n${JSON.stringify(context)}`,
  ...
  async function askText(nextQuestion: string) {
    ...
    setBusy(true)
    try {
      const reply = localMode ? await askLocal(trimmed) : await api.askChibaLab(trimmed, workspace)
      setMessages((prev) => [...prev, { role: 'assistant', text: reply.answer, sources: reply.sources }])
    } catch (err) {
      const message = err instanceof Error ? err.message : 'Ask ChibaLab failed.'
      toast.error(
        !localMode && /Failed to send a request/i.test(message)
          ? 'Ask ChibaLab server is not reachable. Deploy the ask-pearl Edge Function, or turn on Local model and use Ollama.'
          : message,
      )
    ...
    return (
      <section className="no-print fixed bottom-5 right-5 z-40 flex h-[380px] w-[300px] flex-col overflow-hidden rounded-lg border border-ink-200 bg-white dark:border-ink-800 dark:bg-ink-950">
        <header className="flex items-center gap-2 border-b border-ink-200 px-3 py-2 dark:border-ink-800">
          <ChibaLabMark className="h-5 w-5" />
          <div className="min-w-0 flex-1">
            <h2 className="truncate text-sm font-semibold text-ink-900 dark:text-ink-100">Ask ChibaLab</h2>
            <p className="text-[11px] capitalize text-ink-400">{workspace}</p>
          </div>
          <button type="button" className="btn-ghost p-1" aria-label="Close Ask ChibaLab" onClick={() => setState('icon')}>
            <X className="h-4 w-4" />
          </button>
        </header>
  ...
  if (state === 'icon') {
    return (
      <button
        ...
        aria-label="Open Ask ChibaLab"
        onClick={() => setState('panel')}
      >
        <ChibaLabMark className="h-6 w-6" />
      </button>
    )
  }
  ...
  return (
    <div className="no-print fixed bottom-5 right-5 z-40 flex items-center gap-2 rounded-full border border-ink-200 bg-white py-2 pl-2 pr-2 dark:border-ink-800 dark:bg-ink-950">
      <button type="button" className="flex items-center gap-2 pl-1 pr-2" onClick={() => setState('panel')}>
        <ChibaLabMark className="h-6 w-6" />
        <span className="text-sm font-semibold text-ink-800 dark:text-ink-100">Ask ChibaLab</span>
      </button>
      <button type="button" className="btn-ghost rounded-full p-1" aria-label="Collapse Ask ChibaLab" onClick={() => setState('icon')}>
