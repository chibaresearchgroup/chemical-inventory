...
const INQUIRY_OPTIONS: Array<{ value: InquiryType; label: string }> = [
  ...
  { value: 'product', label: 'Use ChibaLab in another lab' },
...
function fallbackMailto(values: {
  ...
  return `mailto:${DEVELOPER_EMAIL}?subject=${encodeURIComponent(values.subject || 'ChibaLab contact')}&body=${encodeURIComponent(body)}`
}
...
export default function ContactDeveloperPage() {
  ...
  const page = typeof window === 'undefined' ? 'ChibaLab' : window.location.href
  ...
  async function submit(e: FormEvent) {
    ...
    try {
      const res = await fetch(FORMSPREE_ENDPOINT, {
        ...
        body: JSON.stringify({
          _subject: subject || `ChibaLab ${type}`,
          type,
          name,
          email,
          message,
  ...
  return (
    <>
      <PageHeader
        ...
        description="Send bugs, feature ideas, customization requests, or product inquiries directly to the ChibaLab developer."
      />
          ...
          <div className="mb-5 flex items-start gap-3">
            ...
            <div>
              <h2 className="text-base font-semibold text-ink-900 dark:text-ink-50">
                Tell me what ChibaLab needs next
              </h2>
