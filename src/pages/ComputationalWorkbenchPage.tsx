...
export default function ComputationalWorkbenchPage() {
  ...
  function toggleViewerMolecule(index: number) {
    ...
    setViewerMolIndexes((prev) => {
      ...
      if (prev.length >= 4) {
        toast.info('ChibaLab renders up to 4 molecules at once. Clear one first.')
        return prev
      }
  ...
  async function sendToHpc() {
    ...
    if (!hpcTarget.trim()) return toast.info('Choose a target folder inside ChibaLab_AGENT_ROOT.')
    setHpcBusy(true)
    try {
  ...
  return (
    <>
      <PageHeader
        ...
        description="Integrated ChibaLab workbench for Gaussian and ORCA inputs, routes, scripts, TICT geometries, SOC preparation, and molecular previews."
        actions={
          <>
              ...
              <div className="grid gap-4 p-4 lg:grid-cols-[minmax(0,1fr)_320px]">
                ...
                <div className="space-y-3">
                  ...
                  <div className="rounded-lg border border-ink-200 p-3 dark:border-ink-800">
                    ...
                    <p className="mt-1 text-xs text-ink-500">Cut old method, basis, solvent, or numbering from uploaded log names before ChibaLab generates new files.</p>
          ...
          <section ref={resultsRef} className="card scroll-mt-24 overflow-hidden">
            ...
            {files.length === 0 ? (
              <div className="flex min-h-[360px] flex-col items-center justify-center gap-3 text-center text-sm text-ink-400">
                ...
                <p>Paste or upload a molecule, then click Generate. ChibaLab will parse it automatically.</p>
              </div>
            ) : (
              <>
              <div className="border-b border-ink-200 p-4 dark:border-ink-800">
                ...
                <p className="mt-2 text-xs text-ink-500">Requires the updated ChibaLab HPC agent running with write mode. Files are written only inside ChibaLab_AGENT_ROOT.</p>
              </div>
