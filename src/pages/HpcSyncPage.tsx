...
function aggregateJobsByFolder(jobs: ManifestJob[], account?: string): ManifestJob[] {
  ...
  return [...map.entries()].map(([folder, rows]) => {
    ...
    return {
      ...
      warnings,
      notes: [
        'Folder/system summary imported by ChibaLab.',
  ...
  return jobs
    .map((job) => {
      ...
      return {
        ...
        metrics: null,
        access_notes: `Raw files stay on Linux/HPC. ChibaLab stores metadata pointers only.${job.account || account ? ` Imported via ${job.account || account}.` : ''}${root ? ` Scan root: ${root}.` : ''}`,
...
export default function HpcSyncPage() {
  ...
  const [terminal, setTerminal] = useState<TerminalLine[]>([
    { kind: 'system', text: 'Start the ChibaLab HPC agent on Linux/HPC, then run shell commands here.' },
  ])
  ...
  return (
    <>
      <PageHeader
        ...
        description="Work console for the read-only ChibaLab agent: inspect folders, scan calculations, and import folder summaries."
        actions={
          <>
          ...
          <div className="overflow-hidden rounded-lg border border-ink-800 bg-ink-950">
            <div className="flex items-center justify-between border-b border-ink-800 px-3 py-2">
              <div className="flex items-center gap-2 text-xs font-semibold text-ink-300">
                <TerminalSquare className="h-4 w-4" /> ChibaLab HPC Terminal
              </div>
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="mt-1 text-xs leading-relaxed text-ink-500">
                ChibaLab stores folder summaries, parsed metadata, paths and ownership. Raw calculation files remain on HPC.
              </p>
            </div>
          </div>
