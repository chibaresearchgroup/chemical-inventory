 ...
 */
// Printing thousands of stickers in one physical batch isn't realistic
// anyway — this also keeps "Select all" from generating that many QR codes
// and DOM nodes at once, which is what was actually freezing the tab.
            ...
            <button
              ...
              onClick={() => {
                ...
                if (allLabels.length > MAX_LABELS_AT_ONCE) {
                  toast.error(
                    `Selected the first ${MAX_LABELS_AT_ONCE} of ${allLabels.length} — printing more than that in one batch would freeze the page. Narrow your search or location filter, then select all again for the rest.`,
                  )
                }
              }}
