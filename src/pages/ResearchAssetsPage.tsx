...
export default function ResearchAssetsPage() {
  ...
  function assetTitle(id: string) {
    ...
    return asset ? `${asset.stable_id ?? 'ChibaLab-RA'} - ${asset.title}` : 'Unknown asset'
  }
  ...
  async function importCodeFiles(files: FileList | null) {
    ...
    try {
      for (const file of selected) {
        ...
        const notes = [
          'Uploaded from local computer as a ChibaLab code/data asset.',
  ...
  return (
    <>
        ...
        actions={
          <>
            ...
            <button
              ...
              title="Draft a methods section from visible ChibaLab metadata"
            ...
            {canEdit && (
              <>
                ...
                <label className="btn-secondary cursor-pointer px-2.5 py-2" title="Add Python, MATLAB, notebook, script or data file as a ChibaLab asset">
                    ...
                    return (
                      <div key={version.id} className="rounded-lg border border-ink-200 p-3 dark:border-ink-800">
                        <div className="flex items-start justify-between gap-3">
                          ...
                          {canEdit && (
                            <button className="btn-ghost py-1.5 text-xs text-rose-600" onClick={() => void removeVersion(version)} title="Remove this ChibaLab version metadata only">
                  ...
                  {[...lineageForDetail.upstream, ...lineageForDetail.downstream].map((link) => (
                    <div key={link.id} className="rounded-lg border border-ink-200 p-3 dark:border-ink-800">
                      <div className="flex items-start justify-between gap-3">
                        ...
                        {canEdit && (
                          <button className="btn-ghost py-1.5 text-xs text-rose-600" onClick={() => void removeLineage(link)} title="Remove this ChibaLab lineage metadata only">
                            <Trash2 className="h-3.5 w-3.5" />
                          </button>
                        )}
                      </div>
                        ...
                        {canEdit && (
                          <div className="flex shrink-0 flex-wrap gap-2">
                            <button
                              type="button"
                              className="btn-secondary py-1.5 text-xs"
                              title="Edit or relabel this ChibaLab record. NSCC files are not touched."
                              onClick={() => {
                                startEdit(row)
                                setFolderDetail(null)
                              }}
                            ...
                            <button
                              ...
                              title="Delete ChibaLab metadata only. NSCC files are not touched."
                              onClick={() => {
      ...
      >
        <div className="space-y-4">
          ...
          <textarea
            ...
            placeholder={busy ? 'Drafting from ChibaLab metadata...' : 'The draft will appear here.'}
          />
        </div>
      </Modal>
      ...
      <ConfirmDialog
        ...
        title="Delete ChibaLab record?"
        destructive
        confirmLabel="Delete record"
        busy={busy}
        message={<p>{confirmDelete?.title} will be removed from ChibaLab only. NSCC/HPC files and folders are not deleted or changed.</p>}
