...
const AGENT_COMMAND = `ChibaLab_AGENT_ROOT=${DEFAULT_AGENT_ROOT} ChibaLab_AGENT_TOKEN=choose-a-secret python3 ~/pearl_hpc_agent.py`
...
export default function HpcTutorialPage() {
  ...
  const directTunnel = `ssh -t -o ExitOnForwardFailure=yes -L 127.0.0.1:${localPort}:127.0.0.1:8787 ${nsccAccount}@aspire2antu.nscc.sg 'cd ${quoteShell(filledRootPath)} && ChibaLab_AGENT_ROOT="$PWD" ChibaLab_AGENT_TOKEN="${token}" python3 ${quoteShell(filledAgentPath)}'`
  const jumpTunnel = `ssh -p ${jumpPort} -t -o ExitOnForwardFailure=yes -L 127.0.0.1:${localPort}:127.0.0.1:8787 ${nsccAccount}@127.0.0.1 'cd ${quoteShell(filledRootPath)} && ChibaLab_AGENT_ROOT="$PWD" ChibaLab_AGENT_TOKEN="${token}" python3 ${quoteShell(filledAgentPath)}'`
  ...
  const customAgentWritable = `ChibaLab_AGENT_ROOT="${filledRootPath}" ChibaLab_AGENT_TOKEN="${token}" ChibaLab_AGENT_ALLOW_WRITES=1 python3 "${filledAgentPath}"`
  const projectFolderShortcut = `cd "${filledRootPath}"\nChibaLab_AGENT_ROOT="$PWD" ChibaLab_AGENT_TOKEN="${token}" python3 "${filledAgentPath}"`
  ...
  return (
    <>
      <PageHeader
        title="HPC Tutorial"
        description="One-time setup for connecting your own Linux/HPC account to ChibaLab without uploading raw calculation files."
        actions={
          <>
      ...
      <div className="grid gap-4 xl:grid-cols-[minmax(0,1.15fr)_minmax(300px,0.85fr)]">
        <section className="card p-5 xl:col-span-2">
          ...
          <div className="grid gap-3 lg:grid-cols-4">
            ...
            <label className="block">
              <span className="mb-1 block text-xs font-semibold uppercase tracking-wide text-ink-400">ChibaLab local port</span>
          ...
          <div className={cx('mt-4 grid gap-3', connection === 'jump' ? 'xl:grid-cols-3' : 'lg:grid-cols-2')}>
            ...
            <QuickCommand
              title={`${connection === 'jump' ? '2' : '1'}. ${osLabel}: ChibaLab tunnel + agent`}
              ...
              code={customTunnel}
              onCopy={() => void copy('ChibaLab tunnel command', customTunnel)}
              copied={copied === 'ChibaLab tunnel command'}
            />
            <QuickCommand
              title={`${connection === 'jump' ? '3' : '2'}. Test on ${osLabel}`}
              text="Run this in a separate local terminal. If it returns JSON, ChibaLab can connect."
              code={healthCommand}
          ...
          <div className="mt-4 rounded-lg border border-pearl-200 bg-pearl-50 p-3 text-sm text-pearl-900 dark:border-pearl-500/25 dark:bg-pearl-500/10 dark:text-pearl-100">
            In ChibaLab Linux/HPC Sync use <span className="font-mono">http://127.0.0.1:{localPort || '8788'}</span>, token <span className="font-mono">{token || 'your token'}</span>, folder <span className="font-mono">.</span>, then run <span className="font-mono">pearl scan .</span>.
          </div>
        </section>
        ...
        <section className="card p-5">
          <div className="grid gap-3 md:grid-cols-3">
            ...
            <GuideStep
              ...
              title="Start"
              text="Run this after SSH login. Replace the scratch path with the exact folder ChibaLab may browse and scan."
              code={AGENT_COMMAND}
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="mt-1 text-sm leading-relaxed text-ink-500">
                The agent is read-only by default. ChibaLab can list folders, preview text files, scan logs, and import metadata. Deleting in ChibaLab only removes the inventory record.
              </p>
            </div>
          </div>
          <div className="mt-4 grid gap-2 text-sm">
            <Safety label="NSCC files" value="Read-only mode never deletes, edits, moves, renames, or uploads files. Write mode only creates generated job files inside ChibaLab_AGENT_ROOT." />
            <Safety label="Generated jobs" value="Workbench can write generated .com/.inp/.sh files only if you start the agent with ChibaLab_AGENT_ALLOW_WRITES=1." />
        ...
        <section className="card p-5 xl:col-span-2">
          ...
          <div className="grid gap-3 md:grid-cols-4">
            ...
            <Workflow title="Scan Folder" text="Click Scan and import. ChibaLab creates folder-level assets, not one noisy card per file." />
            <Workflow title="Browse Assets" text="Open a folder card to see contained records and live folder contents." />
            <Workflow title="Preview Files" text="View text files in ChibaLab, download previews when needed, and keep raw files on HPC." />
          </div>
          <div className="mt-4 rounded-lg border border-amber-200 bg-amber-50 p-3 text-sm text-amber-900 dark:border-amber-500/25 dark:bg-amber-500/10 dark:text-amber-100">
            Example paths use placeholders. A real scratch folder usually looks like <span className="font-mono">/scratch/users/sutd/your_account/project/run_01</span> or <span className="font-mono">/scratch/users/ntu/your_account/project/run_01</span>. Use the folder you want ChibaLab to index.
          </div>
          <div className="mt-3 rounded-lg border border-ink-200 p-3 text-sm dark:border-ink-800">
            <div className="flex flex-wrap items-center justify-between gap-2">
            ...
            <p className="mt-2 text-xs text-ink-500">Use this only when you want the Quantum Input Generator to send generated job files into the selected ChibaLab_AGENT_ROOT folder.</p>
          </div>
          <div className="mt-3 rounded-lg border border-ink-200 p-3 text-sm dark:border-ink-800">
            <div className="flex flex-wrap items-center justify-between gap-2">
            ...
            <p className="mt-2 text-xs text-ink-500">Put this file inside a calculation folder so the ChibaLab scanner imports cleaner project, owner, software, method, notes, and tags.</p>
          </div>
        </section>
      </div>
    </>
