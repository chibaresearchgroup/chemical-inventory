...
export default function MembersPage() {
  ...
  async function sendInvite(e: FormEvent) {
    ...
    try {
      ...
      toast.success(
        `Invite sent to ${inviteEmail}. They can click the email link, or paste the code on the ChibaLab sign-in page.`,
      )
      setInviteName('')
      setInviteEmail('')
    } catch (err) {
  ...
  async function changeRole(target: Profile, role: Role) {
    ...
    if (isDeveloperProfile(target)) {
      toast.error('The ChibaLab developer account cannot be demoted or role-changed.')
      return
    }
  ...
  async function togglePi(target: Profile) {
    if (!profile) return
    if (isDeveloperProfile(target)) {
      toast.error('The ChibaLab developer account is protected from PI controls.')
      return
    }
  ...
  async function revoke(target: Profile) {
    if (!profile) return
    if (isDeveloperProfile(target)) {
      toast.error('The ChibaLab developer account cannot be revoked.')
      return
    }
    if (target.is_pi && !profile.is_pi) {
  ...
  async function removeAccess(target: Profile) {
    if (!profile) return
    if (isDeveloperProfile(target)) {
      toast.error('The ChibaLab developer account cannot be removed.')
      return
    }
    if (target.is_pi && !profile.is_pi) {
  ...
  async function openOffboarding(target: Profile) {
    if (isDeveloperProfile(target)) {
      toast.error('The ChibaLab developer account cannot be offboarded.')
      return
    }
    if (target.is_pi && !profile?.is_pi) {
    ...
    <>
      ...
      <section className="card mb-4 p-4">
        ...
        <p className="mb-3 text-xs text-ink-500 dark:text-ink-400">
          {MODE === 'cloud'
            ? 'Sends a ChibaLab sign-in email. The recipient can click the link, or copy the short code from the email into the ChibaLab sign-in page. The invite is recorded below the moment you send it, so it cannot quietly disappear.'
                    ...
                    <button
                      ...
                      title={
                        piProtected
                          ...
                          : isDeveloper
                            ? 'The ChibaLab developer account cannot be changed by admins or PI users.'
                          : isLastAdmin
