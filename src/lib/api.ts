...
function protectDeveloperAccount(target: Profile, action: string) {
  if (isDeveloperAccount(target)) {
    throw new ApiError(`The ChibaLab developer account cannot be ${action}.`)
  }
}

const LOCAL_LAB_LOCATIONS_KEY = 'pearl.custom_lab_locations'

export interface AskChibaLabReply {
  answer: string
...
function invokeAskChibaLab(body: Record<string, unknown>): Promise<Record<string, unknown>> {
...
export const api = {
  async askChibaLab(question: string, workspace?: 'experimental' | 'computational'): Promise<AskChibaLabReply> {
    if (!IS_CLOUD) {
      return {
        answer:
          'Ask ChibaLab is available when the app is connected to Supabase and a server-side model provider secret is configured.',
        sources: [],
      }
    }
    return (await invokeAskChibaLab({ question, workspace })) as unknown as AskChibaLabReply
  },
  ...
  async draftMethods(project: string): Promise<string> {
    if (!IS_CLOUD) {
      ...
      return `Materials and methods draft for ${project}: ${chemicals.length} chemicals and ${assets.length} research assets are linked in ChibaLab. Review supplier, grade, instrument, and computational parameters before use in a manuscript.`
    }
    const res = await invokeAskChibaLab({ action: 'draft_methods', project })
    ...
    if (!IS_CLOUD) {
      ...
      rows.forEach((r, i) => {
        localDb.insertChemical({
          ...r,
          code: r.code || nextCode([...existing, ...Array.from({ length: i }, (_, k) => `ChibaLab-${k}`)]),
