#!/usr/bin/env python3
"""Small ChibaLab HPC agent.

Run this inside a Linux/HPC account, usually behind an SSH tunnel.
It exposes a local HTTP API that ChibaLab can use to run light shell commands and
...
PORT = int(os.environ.get("ChibaLab_AGENT_PORT", "8787"))
TOKEN = os.environ.get("ChibaLab_AGENT_TOKEN", "")
ROOT = os.path.abspath(os.environ.get("ChibaLab_AGENT_ROOT", os.getcwd()))
MAX_OUTPUT = int(os.environ.get("ChibaLab_AGENT_MAX_OUTPUT", "200000"))
MAX_FILES = int(os.environ.get("ChibaLab_AGENT_MAX_FILES", "8000"))
ACCOUNT_LABEL = os.environ.get("ChibaLab_AGENT_ACCOUNT", getpass.getuser() + "@" + socket.gethostname())
ALLOW_WRITES = os.environ.get("ChibaLab_AGENT_ALLOW_WRITES", "").lower() in ("1", "true", "yes")
...
def inside_root(path):
    ...
    if target == root or target.startswith(root + os.sep):
        return target
    raise ValueError("Path is outside ChibaLab_AGENT_ROOT")
...
def run_command(command, cwd):
    if not ALLOW_WRITES and DANGEROUS_RE.search(command):
        return {
            "code": 126,
            "stdout": "",
            "stderr": "Blocked by ChibaLab read-only mode. Use ls/find/grep/tail/cat/du/pwd/qstat-style commands, or scan a folder. Set ChibaLab_AGENT_ALLOW_WRITES=1 only if you intentionally want write commands.",
            "cwd": cwd,
        }
...
def aggregate_by_folder(files):
    ...
    for folder, rows in sorted(groups.items()):
        ...
        notes = [
            "Folder summary imported by ChibaLab HPC agent.",
...
def write_bundle(target, files, overwrite=False):
    if not ALLOW_WRITES:
        raise ValueError("ChibaLab agent write mode is disabled. Restart the agent with ChibaLab_AGENT_ALLOW_WRITES=1 to send generated files to HPC.")
...
class AgentHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        return response(self, 404, {"error": "Unknown ChibaLab agent route"})
    ...
    def log_message(self, fmt, *args):
        sys.stderr.write("ChibaLab agent: " + (fmt % args) + "\n")


if __name__ == "__main__":
    print("ChibaLab HPC agent listening on http://127.0.0.1:%s" % PORT)
    print("Root: %s" % ROOT)
    if not TOKEN:
        print("No ChibaLab_AGENT_TOKEN set. Use only on trusted localhost/private networks.")
