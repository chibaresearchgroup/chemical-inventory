...
const PORT = Number(process.env.ChibaLab_AGENT_PORT || 8787)
const TOKEN = process.env.ChibaLab_AGENT_TOKEN || ''
const ROOT = resolve(process.env.ChibaLab_AGENT_ROOT || process.cwd())
const MAX_OUTPUT = Number(process.env.ChibaLab_AGENT_MAX_OUTPUT || 200000)
const MAX_FILES = Number(process.env.ChibaLab_AGENT_MAX_FILES || 8000)
const ALLOW_WRITES = ['1', 'true', 'yes'].includes((process.env.ChibaLab_AGENT_ALLOW_WRITES || '').toLowerCase())
...
function insideRoot(path) {
  ...
  throw new Error('Path is outside ChibaLab_AGENT_ROOT')
}
...
http.createServer(async (req, res) => {
  ...
  try {
    ...
    return send(res, 404, { error: 'Unknown ChibaLab agent route' })
  } catch (err) {
    return send(res, 400, { error: err instanceof Error ? err.message : 'Agent failed' })
  }
}).listen(PORT, () => {
  console.log(`ChibaLab HPC agent listening on http://127.0.0.1:${PORT}`)
  console.log(`Root: ${ROOT}`)
  if (!TOKEN) console.log('No ChibaLab_AGENT_TOKEN set. Use only on trusted localhost/private networks.')
})
